const esMobil = window.matchMedia('(hover: none) and (pointer: coarse)').matches;

/*
==============================================================================
AUDIO — Pool de instancias pre-cargadas para evitar lag de instanciación
==============================================================================
*/
/**
 * Crea un pool de N instancias de Audio listas para ser reutilizadas.
 * Ideal para sonidos que se disparan frecuentemente (propulsión, disparos).
 * @param {string} src  Ruta del archivo de audio
 * @param {number} size Tamaño del pool (instancias simultáneas)
 * @param {number} vol  Volumen 0-1
 * @returns {Function} Función que reproduce el siguiente audio disponible
 */
function crearPool(src, size, vol = 1) {
...
// En móvil la propulsión se silencia: se dispara cada ~80ms por nave → muy costoso
const reproducirPropulsion = esMobil
  ? () => {}
  : crearPool('explosion.wav', 3, 0.1);

const reproducirSonido   = crearPool('explosion.wav', 4, 0.6);
const reproducirDisparo  = crearPool('missile_launcher.wav', 3, 1.0);
const reproducirRecolect = crearPool('coin_recolect_s.wav', 2, 1.0);

// La música solo se reproduce una vez, no necesita pool
